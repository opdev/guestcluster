...
// computeLeaseAccounting derives, from every ClusterLease in a namespace,
...
// lease and never touches the instance, so this accounting considers an
...
// there is no window where the claim exists on one side but not the other
// for this accounting to observe.
//
// pendingDemand counts, in the same pass, non-terminal ClusterLeases that
// target poolName (same PoolRef) and have not yet claimed an instance.
// ClusterLeaseReconciler is pure demand-side matchmaking: it never
// provisions instances itself (see clusterlease_controller.go). Without
// pendingDemand, a pool with MinSize=0 and WarmSpares=0 (pure on-demand)
// would never create an instance for a Pending lease, because `need` in
// Reconcile would always be 0 with no warm floor to maintain. pendingDemand
// makes MinSize=0, WarmSpares=0 a valid, working configuration instead of a
// deadlock.
func computeLeaseAccounting(leaseList *brokerv1alpha1.ClusterLeaseList, poolName string) (claimed map[string]bool, pendingDemand int32) {
...
// MinSize/WarmSpares floors are reachable given MaxSize, and whether
// current demand can be satisfied within it.
func capacityCondition(pool *brokerv1alpha1.ClusterPool, need, room, pendingDemand int32) metav1.Condition {
	switch {
...
// scaleDownExcess deletes at most one Ready, unclaimed ClusterInstance when
// available exceeds every floor, applying the same
// thundering-herd-avoidance policy as the top-up path in Reconcile: at most
// one instance per reconcile. handled reports whether the caller should
// return result and err immediately; when handled is false, there is no
// excess and the caller should continue with its own status update.
//
// An instance is excess only if deleting it would not violate either floor:
//   - spareFloor (raised above WarmSpares by outstanding pendingDemand, for
//     the same reason as warmSpareNeed/demandNeed in Reconcile): without
//     this floor, a pool with WarmSpares=0 would target the very instance
//     it just provisioned for a Pending lease as excess the instant that
//     instance turned Ready, before ClusterLeaseReconciler got a chance to
//     claim it.
//   - MinSize, measured against total (not available): total does NOT
//     change when a lease claims an instance, so this floor is immune to
//     the same accounting race.
//
// excess is the minimum of what each floor allows trimming. Among the
// excess candidates, only instances that have been sitting
// Ready-and-unclaimed for at least scaleDownStabilityPeriod are eligible
// for deletion this reconcile; see scaleDownStabilityPeriod's doc.
// instanceIdleSince uses the Ready condition's LastTransitionTime. A
// claimed instance is deleted outright on release rather than ever
...
func (r *ClusterPoolReconciler) scaleDownExcess(ctx context.Context, pool *brokerv1alpha1.ClusterPool, availableInstances []*brokerv1alpha1.ClusterInstance, available, total, pendingDemand int32) (ctrl.Result, bool, error) {
	...
	if len(eligible) == 0 {
		// Nothing has cleared the stability window yet; come back once the
	...
	if err := r.Delete(ctx, victim); err != nil && !apierrors.IsNotFound(err) {
		return ctrl.Result{}, true, fmt.Errorf("deleting excess ClusterInstance %s for pool %s: %w", victim.Name, pool.Name, err)
	}
	// This is the same read-your-own-write cache race as the top-up path in
	// Reconcile: an immediate Requeue here could re-evaluate `excess`
	// against a cache that has not yet observed this Delete(), over-trimming
