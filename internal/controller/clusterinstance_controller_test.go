...
var _ = Describe("ClusterInstance Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind ClusterInstance")
			err := k8sClient.Get(ctx, typeNamespacedName, clusterinstance)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ClusterInstance")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &ClusterInstanceReconciler{
				Client: k8sClient,
