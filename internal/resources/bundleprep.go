...
// bundle-prep Job runs as. This indirection exists because config/default
...
const BundlePrepServiceAccountEnvVar = "BUNDLE_PREP_SERVICE_ACCOUNT"

// DefaultBundlePrepServiceAccount is used when BundlePrepServiceAccountEnvVar
// is unset, matching config/rbac/bundle_prep_service_account.yaml's
// unprefixed name (for example when running via `make run` outside a Pod).
const DefaultBundlePrepServiceAccount = "bundle-prep"

// BundlePrepServiceAccount resolves the name of the ServiceAccount the
...
// BundlePrepServiceAccount prefers BundlePrepServiceAccountEnvVar, falling
// back to DefaultBundlePrepServiceAccount when unset.
func BundlePrepServiceAccount() string {
	if sa := os.Getenv(BundlePrepServiceAccountEnvVar); sa != "" {
		return sa
	}
	return DefaultBundlePrepServiceAccount
}
