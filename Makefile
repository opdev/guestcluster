...
# opdev.io/guestcluster-operator-bundle:$VERSION and opdev.io/guestcluster-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= opdev.io/guestcluster-operator
...
# Generates a single Markdown reference doc (docs/reference/crd-api.md) covering
# every CRD in api/v1alpha1, from the Go doc comments and kubebuilder markers in
# api/v1alpha1/*_types.go. Run manually and commit the result after changing the
# API types; this is not enforced in CI.
.PHONY: api-docs
api-docs: crd-ref-docs ## Generate the CRD API reference doc into docs/reference/.
	@mkdir -p docs/reference
	$(CRD_REF_DOCS) --source-path=./api/v1alpha1 --renderer=markdown \
		--config=hack/crd-ref-docs-config.yaml \
		--output-path=docs/reference/crd-api.md
...
KIND_CLUSTER ?= guestcluster-operator-test-e2e
...
test-e2e: setup-test-e2e manifests generate fmt vet ## Run the e2e tests. Expected an isolated environment using Kind.
	KIND_CLUSTER=$(KIND_CLUSTER) go test ./test/e2e/ -timeout=40m -v -ginkgo.v
...
# CRC_AGENT_IMG is the image used for the per-ClusterInstance crc-agent Job
# and (reused) the CRCBundle bundle-prep Job. See Dockerfile.crc-agent.
CRC_AGENT_IMG ?= opdev.io/guestcluster-operator-crc-agent:latest

.PHONY: docker-build-crc-agent
docker-build-crc-agent: ## Build the crc-agent (and bundle-prep) docker image.
	$(CONTAINER_TOOL) build -t ${CRC_AGENT_IMG} -f Dockerfile.crc-agent .

.PHONY: docker-push-crc-agent
docker-push-crc-agent: ## Push the crc-agent (and bundle-prep) docker image.
	$(CONTAINER_TOOL) push ${CRC_AGENT_IMG}
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name guestcluster-operator-builder
	$(CONTAINER_TOOL) buildx use guestcluster-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm guestcluster-operator-builder
	rm Dockerfile.cross
